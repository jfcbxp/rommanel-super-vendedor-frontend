...
export const useSchedulingTotalizerService = () => {
    ...
    const getDaily = async (code: string): Promise<SchedulingTotalizer> => {
        ...
        const response: AxiosResponse<SchedulingTotalizer> = await http(auth?.token).get<SchedulingTotalizer>(`${resourceURL}/diario/${code}`)
        return response.data
    }

    const getMontly = async (code: string): Promise<SchedulingTotalizer> => {
        let auth = await context.validateToken().then(token => { return token })
        const response: AxiosResponse<SchedulingTotalizer> = await http(auth?.token).get<SchedulingTotalizer>(`${resourceURL}/mensal/${code}`)
        return response.data
    }
